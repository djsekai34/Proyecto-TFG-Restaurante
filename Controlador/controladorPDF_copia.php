...
$horaCobroParaMostrar  = str_replace('-', ':', $data["horaCobro"]);
...
$mpdf->WriteHTML('
    <br><br>
<h3 style="text-align: center;">Mesa: <span style="color: #FF4500;">' . $data["nombreTicket"] . '</span>&nbsp;&nbsp;Camarero: <span style="color: #FF4500;">' . $camarero . '</span>&nbsp;&nbsp;Fecha y Hora: <span style="color: #FF4500;">' . $data["fechaCobro"] . ' / ' . $horaCobroParaMostrar . '</span></h3> <br>
...
// Define el nombre del archivo de la copia del PDF
$nombreArchivoCopia = 'Ticket_' . $data["nombreTicket"] . '_' . $camarero . '_' . $data["fechaCobro"] . '_' . $data["horaCobro"] . '.pdf';

// Verifica si el directorio existe, y si no, lo crea
$directory = '../CopiasVentas/';
if (!is_dir($directory)) {
    mkdir($directory, 0777, true);
}

// Guarda la copia del PDF en la carpeta 'CopiasVentas'
file_put_contents($directory . $nombreArchivoCopia, $pdf);

// Elimina el archivo original de la carpeta 'Comanda'
unlink('../Comanda/' . $data['nombreArchivo']);
?>
